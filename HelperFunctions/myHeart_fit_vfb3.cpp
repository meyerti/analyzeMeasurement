...
void myHeart_Fit_v0b_Range(double *para, int ma, int *ia, int mfit){
    ...
    i=1;
    if(ia[i]>1)myRangeCheck(ia[i]-1,gl_pFit[i+7],gl_pFit[i+7+7],para[i],para);
    if((ia[i]!=2&&ia[i]!=4&&ia[i]!=5&&ia[i]!=13)&&para[1]<=para[0])para[1]=para[0]+1e-17;  // A2 shall be >A1
    i=2;
    if(ia[i]>1)myRangeCheck(ia[i]-1,gl_pFit[i+7],gl_pFit[i+7+7],para[i],para);
    if((ia[i]!=2&&ia[i]!=4&&ia[i]!=5&&ia[i]!=13)&&para[2]<=para[1])para[2]=para[1]+1e-17;  // A3 shall be >A2
